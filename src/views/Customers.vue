...
export default defineComponent({
  ...
  data() {
    return {
      ...
      collectors: [],
      role: '',
      selectedCollector: this.loan.collector?.id
    };
  },
  methods: {
    showToast() {
    ...
    addComment() {
      ...
      axios
        .post(
          `${this.base_url}/staff/addcomment/`,
          {
    ...
    updateCollector() {
      axios
        .post(`${this.base_url}/staff/assign-collector/`, {
        ...
        .then((response) => {
          ...
          // Optionally, update the loan object in your UI
        })
        .catch((error) => {
          // Handle error
  ...
  components: { Modal },
  mounted() {
    const hooks = useTableData();
    hooks.fetchCollector();
    this.collectors = hooks.Collectors;
...
<template>
  <div
    v-if="role == 'admin'"
    class="w-full my-5 px-2 py-4 border rounded-lg bg-white mx-auto shadow-lg"
  >
    <div class="block">
      <label class="block font-bold" for="">Change Collector</label>
      <select
        class="form-control"
        name="col"
        id="col"
        v-model="selectedCollector"
        :onchange="updateCollector" 
      >
        ...
        <option
          ...
          :value="`${collector.id}`"
        >
          {{ collector.user.phone ?? collector.username }}
        </option>
      </select>
    </div>
  </div>
  <div class="flex w-full">
    <div
      class="w-1/3 px-2 border rounded-lg bg-white mx-auto shadow-lg max-w-xs"
    >
      ...
      <div class="flex justify-between border-b-2 mb-1">
        ...
        <div>
          {{ loan.get_due_payment[1] }} day{{
            loan.get_due_payment[1] > 1 ? 's' : ''
          }}
        </div>
      </div>
      <div class="flex justify-between border-b-2 mb-1">
      ...
      <div class="flex justify-between border-b-2 mb-1">
        ...
        <div>
          {{ loan.customer?.cards[loan?.customer?.cards?.length - 1]?.issuer }}
        </div>
      </div>
      <div class="flex justify-between border-b-2 mb-1">
        <div class="font-bold">Card type:</div>
        <div>
          {{ loan?.customer?.cards[loan?.customer?.cards?.length - 1]?.type }}
        </div>
      </div>
      <div class="flex justify-between border-b-2 mb-1">
        ...
        <div>
          {{
            loan?.customer?.cards[loan?.customer?.cards?.length - 1]
              ?.first_6digits
          }}
          ****
          {{
      ...
      <div class="flex justify-between border-b-2 mb-1">
        ...
        <div>
          {{ loan?.customer?.cards[loan?.customer?.cards?.length - 1]?.expiry }}
        </div>
      </div>
    </div>
    <div
  ...
  <div class="mt-8">
    <div class="flex flex-col w-full">
      <div class="flex justify-between border-b-2 mb-1 w-full bg-blue-200">
        <div
          @click="tabitem = 'collection'"
          :class="
            tabitem == 'collection'
              ? 'bg-blue-800 text-center w-1/3  py-2 text-white'
              : 'bg-transparent  text-center w-1/3  py-2 text-blue-900 cursor-pointer'
          "
        >
          Collection
        </div>
        <div
          @click="tabitem = 'history'"
          :class="
            tabitem == 'history'
              ? 'bg-blue-800 text-center w-1/3  py-2 text-white'
              : 'bg-transparent  text-center w-1/3  py-2 text-blue-900 cursor-pointer'
      ...
      <div class="flex flex-col w-full">
        <div
          :class="
            tabitem == 'collection' ? 'flex my-10 px-5 overflow-auto' : 'hidden'
          "
        >
          <!-- form -->
          <div class="flex w-full">
              ...
              >
                ...
                >
                  ...
                  <path
                    ...
                    stroke="url(#paint0_linear_274_857)"
                    stroke-width="14"
        ...
        <table
          class="border border-slate-400 w-full whitespace-nowrap overflow-scroll"
        >
          <thead class="">
            <tr>
              <th
          ...
          <tbody>
            <tr v-for="(comment, index) in loan.customer.comments" :key="index">
              <td
                class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1 font-bold"
              >
                {{ comment.collection_type }}
              </td>
              <td
                class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
              >
                {{ comment.collection_object }}
              </td>
              <td
                class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
              >
              ...
              <td
                class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
              >
                {{ comment.collection_contact }}
              </td>

              <td
                class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
              >
                {{ comment.collection_status }}
              </td>
            </tr>
          </tbody>
        </table>
        <div
          :class="
            tabitem == 'history' ? 'flex my-10 px-5 overflow-auto' : 'hidden'
          "
        >
          <table
            class="border border-slate-400 w-full whitespace-nowrap overflow-scroll"
            ...
            <tbody>
              <tr v-for="(loanitem, index) in loan.customer.loans" :key="index">
                ...
                <td
                  class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
                >
                  {{ loanitem.get_due_payment[0] }}
                </td>
                <td
                  class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
                >
                  {{ loanitem.total_repayment }}
                </td>
                <td
                  class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
                >
                  {{ new Date(loanitem.accept_date).toLocaleDateString() }}
                </td>
                <td
                  class="px-6 py-4 border-b border-gray-200 whitespace-nowrap p-1"
                >
        ...
        <div
          :class="
            tabitem == 'repayment' ? 'flex my-10 px-5 overflow-auto' : 'hidden'
          "
        >
          <table
            class="border border-slate-400 w-full whitespace-nowrap overflow-scroll"
