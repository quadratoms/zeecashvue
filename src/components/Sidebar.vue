    ...
    <div :class="isOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
      ...
      <div class="flex items-center justify-center mt-8">
        <div class="flex items-center">
          ...
          <span class="mx-2 text-2xl font-semibold text-white">ZeeCash</span>
        </div>
      </div>
      ...
      <nav class="mt-10">
        ...
        <div @click="() => router.push('/tables/' + r)" v-for="(r, index) in customerRoute" :key="index"
          class="flex items-center px-6 py-2 mt-4 duration-200 border-l-4"
          :class="r == name ? activeClass : inactiveClass">
          <svg class="w-5 h-5" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
          ...
          <span @click="hello()" class="mx-4">{{ r }}</span>
        </div>
...
// const c=router.currentRoute.value
